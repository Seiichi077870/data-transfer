    ...
    With wsNew1
        ...
        For i = itemRow + 1 To lastRow
            ...
            For j = 2 To lastRow2 ' ヘッダー行をスキップするため2から開始
                If wsNew2.Cells(j, 5).Value = partNo And Not dict.Exists(j) Then ' E列は5列目
                    If wsNew2.Cells(j, 9).Value = quantity Then ' I列：員数
                        isFound = True
...
Sub ファイル保存(wbNew As Workbook, file2 As String)
    ...
    'MsgBox "ファイルを保存しました: " & folderPath & newFileName, vbInformation
End Sub
